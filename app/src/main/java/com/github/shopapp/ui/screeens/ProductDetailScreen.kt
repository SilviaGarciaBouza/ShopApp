...
@Composable
fun ProductDetailScreen(onb:()->Unit,amount: Double,product: Product, onBackClick:()->Unit,onAddtoCart:(Product, Int)->Unit){
    Scaffold(
        topBar = {TipAppBar(onb={onb()},amount= amount,modifier = Modifier, true, onBackClick,{})}
    ) {
        paddingValues ->
        ProductDetailComponent(product= product, modifier= Modifier.padding(paddingValues), onAddtoCart)
    }
}